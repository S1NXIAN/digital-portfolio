/**
 * Seed script — populates the portfolio database with professional sample data.
...
async function main() {
  ...
  console.log(
    `Seeded: skills=${skills.length}, experiences=${experiences.length}, repos=${repos.length}, knowledge=${knowledge.length}, contributions=${rows.length}`
  );
}
